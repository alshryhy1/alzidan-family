...
function ensureDelegateIncomingEventRequestsCard() {
  ...
  if (eventsCard && eventsCard.parentNode === dashboardCard) {
    dashboardCard.insertBefore(card, eventsCard);
  } else {
...
function parseEventMessage(msg) {
  ...
  if (idx !== -1) {
    ...
    try {
      ...
      if (first !== -1 && last > first) {
        let chunk = after.slice(first, last + 1);
        try {
          payload = JSON.parse(chunk);
        } catch (_) {
          try { payload = JSON.parse(chunk.replace(/\\"/g, '"')); } catch (__) {}
        }
      }
    }
  }
  ...
  const type = String(event.type || event.typeLabel || getLabel("نوع المناسبة") || j.type || "").trim();
  const person = String(event.person || getLabel("اسم صاحب المناسبة") || j.person || "").trim();
  const dateLabel = String(event.dateLabel || event.date_label || getLabel("التاريخ") || getLabel("تاريخ المناسبة") || j.dateLabel || "").trim();
  const eventDate = String(event.eventDate || event.event_date || j.eventDate || "").trim();

  const imageUrl = String(media.imageUrl || media.image_url || event.imageUrl || getLabel("الصورة") || getLabel("رابط الصورة") || "").trim();
  const videoUrl = String(media.videoUrl || media.video_url || event.videoUrl || getLabel("رابط الفيديو") || getLabel("الفيديو") || "").trim();

  let details = String(event.text || event.extra || j.text || "").trim();
  ...
  if (!details) {
    details = raw
      ...
      .filter((x) =>!/الصورة\s*:|رابط الصورة\s*:|الفيديو\s*:|رابط الفيديو\s*:|بيانات المرسل\s*:|البريد\s*:|الجوال\s*:|رقم الطلب\s*:|الفرع\s*:|التاريخ\s*:|نوع المناسبة\s*:|اسم صاحب المناسبة\s*:/i.test(x))
      .join(" · ");
  }
  ...
  return {
    ...
    imageUrl,
    videoUrl,
    submitterName: String(submitter.name || j.submitterName || getLabel("الاسم") || "").trim(),
    submitterPhone: String(submitter.phone || j.submitterPhone || getLabel("الجوال") || "").trim(),
    submitterEmail: String(submitter.email || j.submitterEmail || getLabel("البريد") || "").trim()
  };
}
...
function eventTypeArabicLabel(type) {
  ...
  return map[String(type || "").trim()] || "مناسبة عامة";
}

function normalizeEventType(raw) {
  const map = {
    birth: "birth",           "مولود": "birth",
    marriage: "marriage",     "زواج": "marriage",
    contract: "marriage",     "عقد قران": "marriage",
    engagement: "marriage",   "خطوبة": "marriage",
    graduation: "graduation", "تخرج": "graduation",
    promotion: "promotion",   "ترقية": "promotion",
    "ترقية / وظيفة": "promotion",
    gathering: "gathering",   "اجتماع": "gathering",
    "اجتماع عائلي": "gathering",
    sick: "sick",             "مريض": "sick",
    death: "death",           "وفاة": "death",
    success: "general",       "نجاح": "general",
    new_house: "general",     "منزل جديد": "general",
    happy: "general",
  };
  return map[(raw || "").trim()] || "general";
}
...
function renderDelegateIncomingEventRequests(data) {
  ...
  filtered.forEach(req => {
    const parsed  = parseEventMessage(req.message);
    ...
    item.innerHTML = `
      ...
      ${parsed.type ? `<div style="font-size:13px;color:#047857;margin-top:2px;">نوع المناسبة: <strong>${escapeHtml(eventTypeArabicLabel(normalizeEventType(parsed.type)))}</strong></div>` : ""}
    ...
    pubBtn.addEventListener("click", async () => {
      ...
      const row = {
        type:    normalizeEventType(parsed.type || req.kind),
        person:  parsed.person || req.name || "",
        details: parsed.details || req.message || "",
      };
      if (parsed.dateLabel) row.date_label = parsed.dateLabel;
      if (parsed.eventDate) row.event_date = parsed.eventDate;
      ...
      const { data: done, error: rpcErr } = await sb.rpc(
        "delegate_set_approval_request_status_v1",
        { p_branch_key: req.branch_key, p_request_id: Number(req.id), p_status: "approved" }
      );
      if (rpcErr || done === false) {
        showAlert(inlineAlert, "error",
          "نُشرت المناسبة لكن تحديث حالة الطلب فشل: " + ((rpcErr && rpcErr.message) || ""));
        lock(false); return;
      }
      showAlert(inlineAlert, "success", "✓ نُشرت المناسبة بنجاح.");
    ...
    rejBtn.addEventListener("click", async () => {
      ...
      const { data: done, error: rpcErr } = await sb.rpc(
        "delegate_set_approval_request_status_v1",
        { p_branch_key: req.branch_key, p_request_id: Number(req.id), p_status: "rejected" }
      );
      if (rpcErr || done === false) {
        showAlert(inlineAlert, "error",
          "تعذر رفض الطلب: " + ((rpcErr && rpcErr.message) || "تحقق من الصلاحية."));
        lock(false); return;
      }
      item.style.opacity = "0.4";
      item.style.pointerEvents = "none";
...
function parseWifeFamilyValue() {
  const v = wifeFamily ? String(wifeFamily.value || "").trim() : "";
  if (v === "true") return true;
  if (v === "false") return false;
  return null;
}
function setWifeAlert(type, text) {
  if (!wifeAlert) return;
  wifeAlert.textContent = text || "";
...
let wifeChildrenManagerState = { spouse: null, children: [], linked: new Set() };

async function getSelectedParentChildrenForWifeManager() {
  const sb = getالخدمةClient();
  const parentId = resolveSelectedParentId(
    normalizePersonName(parentSelect && parentSelect.value ? parentSelect.value : ""),
    state.branch
  );
  ...
  if (error) {
    setWifeAlert("error", "تعذر تحميل أبناء الأب للربط.");
    return [];
  }
...
async function loadLinkedChildrenForSpouse(spouseId) {
  const sb = getالخدمةClient();
...
function ensureWifeChildrenManagerBox() {
  let box = document.getElementById("wife-children-manager");
  if (box) return box;
  box = document.createElement("div");
  box.id = "wife-children-manager";
  box.className = "field";
  box.style.marginTop = "10px";
  if (wivesList && wivesList.parentElement) {
    wivesList.parentElement.insertBefore(box, wivesList.nextSibling);
  }
  return box;
}

function closeWifeChildrenManager() {
  wifeChildrenManagerState = { spouse: null, children: [], linked: new Set() };
  renderWifeChildrenManager();
}

function renderWifeChildrenManager() {
  const box = ensureWifeChildrenManagerBox();
  if (!box) return;

  const spouse = wifeChildrenManagerState.spouse;
  if (!spouse) {
    box.innerHTML = "";
  ...
  const children = wifeChildrenManagerState.children || [];
  const linked = wifeChildrenManagerState.linked || new Set();
  ...
  box.innerHTML =
    ...
    (children.length ? children.map((child) => {
      const checked = linked.has(String(child.id)) ? " checked" : "";
      return '<label style="display:flex; align-items:center; gap:8px; padding:6px 0; border-bottom:1px solid #eee;">' +
        '<input type="checkbox" data-wife-child-id="' + String(child.id) + '"' + checked + ' />' +
        '<span>' + escapeHtml((child.label || child.name) + (child.order ? " — الترتيب: " + child.order : "")) + '</span>' +
        '</label>';
    }).join("") : '<div class="hint">لا يوجد أبناء لهذا الأب يمكن ربطهم.</div>') +
    '</div>' +
    '<div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:8px;">' +
    '<button id="save-wife-children-links-btn" class="btn btn-primary btn-small" type="button">حفظ ربط الأبناء</button>' +
    '<button id="cancel-wife-children-links-btn" class="btn btn-secondary btn-small" type="button">إغلاق</button>' +
    '</div>';

  const saveBtn = document.getElementById("save-wife-children-links-btn");
  const cancelBtn = document.getElementById("cancel-wife-children-links-btn");
...
async function openWifeChildrenManager(spouseRow) {
  const spouseId = spouseRow && spouseRow.id != null ? Number(spouseRow.id) : 0;
  if (!spouseId) return setWifeAlert("error", "تعذر تحديد الزوجة.");

  const children = await getSelectedParentChildrenForWifeManager();
  const linked = await loadLinkedChildrenForSpouse(spouseId);

  wifeChildrenManagerState = { spouse: spouseRow, children, linked };
  renderWifeChildrenManager();
}
...
async function saveWifeChildrenLinks() {
  const sb = getالخدمةClient();
  const spouse = wifeChildrenManagerState.spouse;
  if (!sb || !spouse || spouse.id == null) return setWifeAlert("error", "تعذر حفظ الربط.");

  const spouseId = Number(spouse.id);
  const childIds = (wifeChildrenManagerState.children || []).map((c) => String(c.id)).filter(Boolean);
  const checkedIds = new Set(Array.from(document.querySelectorAll("[data-wife-child-id]"))
    .filter((el) => el.checked)
    .map((el) => String(el.getAttribute("data-wife-child-id") || "").trim())
    .filter(Boolean));

  if (childIds.length) {
    const del = await sb.from("tree_mother_links").delete().in("child_id", childIds).eq("spouse_id", spouseId);
    if (del.error) return setWifeAlert("error", "تعذر تحديث الربط: " + (del.error.message || "خطأ غير معروف"));
  }

  const selectedChildren = (wifeChildrenManagerState.children || []).filter((c) => checkedIds.has(String(c.id)));
  if (selectedChildren.length) {
    ...
    if (ins.error) return setWifeAlert("error", "تعذر حفظ ربط الأبناء: " + (ins.error.message || "خطأ غير معروف"));
  }

  setWifeAlert("success", "تم حفظ ربط أبناء الزوجة.");
...
function fillWifeFormForEdit(row) {
  if (!row) return;
  if (wifeName) wifeName.value = row.wife_name || "";
  if (wifeFamily) wifeFamily.value = row.wife_is_family_member === false ? "no" : "yes";
  if (wifeBranch) wifeBranch.value = row.wife_branch_key || "";
...
async function updateWifeIfEditing(row) {
  const id = Number(window.__editingTreeSpouseId || 0);
  if (!id) return false;
  const sb = getالخدمةClient();
  if (!sb) return false;

  const { error } = await sb.from("tree_spouses").update(row).eq("id", id);
  if (error) {
    setWifeAlert("error", "تعذر تعديل الزوجة: " + (error.message || "خطأ غير معروف"));
    return true;
  }

  window.__editingTreeSpouseId = "";
  setWifeAlert("success", "تم تعديل بيانات الزوجة.");
  await loadWivesForSelectedParent();
  return true;
}
...
function renderWivesRows(rows) {
  ...
  if (!list.length) {
    ...
    empty.textContent = "لا توجد زوجات مسجلة لهذا الأب حالياً.";
  ...
  list.forEach((row) => {
    ...
    if (childWifeSelect && wifeId != null) {
      ...
      opt.value = String(wifeId);
      opt.textContent = wifeText || ("زوجة رقم " + String(wifeId));
      childWifeSelect.appendChild(opt);
    }
    const item = document.createElement("div");
    item.className = "child-row";
    ...
    info.innerHTML =
      '<strong>' + (wifeText || "بدون اسم") + '</strong>' +
      '<div class="hint">الترتيب: ' + (row.marriage_order || "غير محدد") +
      ' — الأبناء المرتبطون: ' + (row.linked_children_count || 0) + '</div>';
    item.appendChild(info);
    const manageBtn = document.createElement("button");
    manageBtn.type = "button";
    manageBtn.className = "btn btn-secondary btn-small";
    manageBtn.textContent = "إدارة أبناء الزوجة";
    manageBtn.setAttribute("data-manage-wife-children", "1");
    manageBtn.style.marginTop = "8px";
    manageBtn.addEventListener("click", () => openWifeChildrenManager(row));
    item.appendChild(manageBtn);
    if (!item.querySelector("[data-manage-wife-children]")) {
      const manageBtn = document.createElement("button");
      manageBtn.type = "button";
      manageBtn.className = "btn btn-secondary btn-small";
      manageBtn.textContent = "إدارة أبناء الزوجة";
    ...
    if (!item.querySelector("[data-edit-wife]")) {
      const editWifeBtn = document.createElement("button");
      editWifeBtn.type = "button";
      editWifeBtn.className = "btn btn-secondary btn-small";
      editWifeBtn.textContent = "تعديل الزوجة";
      editWifeBtn.setAttribute("data-edit-wife", "1");
      editWifeBtn.style.marginTop = "8px";
      editWifeBtn.style.marginInlineStart = "8px";
      editWifeBtn.addEventListener("click", () => fillWifeFormForEdit(row));
      item.appendChild(editWifeBtn);
    }

    wivesList.appendChild(item);
  });
}
async function loadWivesForSelectedParent() {
  ...
  const r = await sb
    ...
    .select("id,husband_id,wife_name,wife_is_family_member,wife_branch_key,wife_lineage,marriage_order,status,confidence,linked_children_count")
...
async function saveWifeForSelectedParent() {
  if (!state.branch) {
    setWifeAlert("error", "يلزم تسجيل دخول المندوب أولاً.");
    return;
  }
  const sb = getالخدمةClient();
  if (!sb) {
    setWifeAlert("error", "تعذر الاتصال بقاعدة البيانات.");
    return;
  }
  const parentName = resolveSelectedParentId(normalizePersonName(parentSelect.value), state.branch);
  if (!parentName) {
    setWifeAlert("error", "اختر الأب أولاً.");
    return;
  }
  const husbandId = await getTreePersonIdByName(sb, parentName);
  if (!husbandId) {
    setWifeAlert("error", "تعذر تحديد رقم الأب في قاعدة البيانات.");
    return;
  }
  const name = normalizePersonName(wifeName && wifeName.value ? wifeName.value : "");
  if (!name) {
    setWifeAlert("error", "أدخل اسم الزوجة.");
    return;
  }
  const orderRaw = wifeOrder ? normalizeArabicDigitsToLatin(String(wifeOrder.value || "").trim()) : "";
  const order = orderRaw ? parseInt(orderRaw, 10) : null;
  if (orderRaw && (!order || order < 1 || order > 4)) {
    setWifeAlert("error", "ترتيب الزوجة يجب أن يكون من 1 إلى 4.");
    return;
  }
  const row = {
    husband_id: husbandId,
    wife_name: name,
    wife_is_family_member: parseWifeFamilyValue(),
    wife_branch_key: wifeBranch && wifeBranch.value ? String(wifeBranch.value).trim() : null,
    wife_lineage: wifeLineage && wifeLineage.value ? normalizePersonName(wifeLineage.value) : null,
  ...
  if (await updateWifeIfEditing(row)) return;
  const r = await sb.from("tree_spouses").insert(row).select("id").single();
  if (r.error) {
    setWifeAlert("error", "تعذر حفظ الزوجة: " + (r.error.message || "خطأ غير معروف"));
    return;
  }
  if (wifeName) wifeName.value = "";
  if (wifeFamily) wifeFamily.value = "";
  if (wifeBranch) wifeBranch.value = "";
...
async function confirmLinkAllChildrenToOnlyWife() {
  ...
  const husbandId = await getTreePersonIdByName(sb, parentName);
  if (!husbandId) {
    setWifeAlert("error", "تعذر تحديد رقم الأب.");
    return;
  }
  const ok = window.confirm("تأكيد مهم: سيتم ربط كل أبناء هذا الأب بزوجته الوحيدة المسجلة. هل أنت متأكد؟");
  if (!ok) return;
  const r = await sb.rpc("confirm_link_all_children_to_only_spouse", { p_husband_id: husbandId });
  if (r.error) {
    setWifeAlert("error", r.error.message || "تعذر الربط الجماعي.");
    return;
  }
...
function ensureDelegateChildCardsLayout() {
  const name = document.getElementById("child-name-1");
  if (!name || document.getElementById("delegate-first-child-card")) return;
  ...
  const firstField = fieldOf("child-name-1");
  if (!firstField || !firstField.parentElement) return;

  const card = document.createElement("div");
  card.id = "delegate-first-child-card";
  card.style.cssText = "grid-column:1/-1;border:1px solid #d8e8df;border-radius:16px;padding:14px;margin:8px 0;background:#fff;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;";

  const title = document.createElement("div");
  title.textContent = "الابن الأول";
  title.style.cssText = "grid-column:1/-1;font-weight:800;color:#064e3b;margin-bottom:4px;";
  card.appendChild(title);

  firstField.parentElement.insertBefore(card, firstField);

  ids.forEach((id) => {
    const f = fieldOf(id);
    if (f && f !== card && f.parentElement) card.appendChild(f);
  });
}

function labelDelegateExtraChildCards() {
  document.querySelectorAll("[data-extra-child-row]").forEach((row, index) => {
    if (!row || row.dataset.childCardLabeled === "1") return;
    row.dataset.childCardLabeled = "1";
    row.style.border = "1px solid #d8e8df";
    ...
    const title = document.createElement("div");
    title.textContent = "الابن " + String(index + 2);
    title.style.cssText = "grid-column:1/-1;font-weight:800;color:#064e3b;margin-bottom:4px;";
    row.insertBefore(title, row.firstChild);
  });
}

function refreshDelegateChildCardsLayout() {
  ensureDelegateChildCardsLayout();
  enhanceDelegateExtraChildRows();
  labelDelegateExtraChildCards();
}

function enhanceDelegateExtraChildRows() {
  document.querySelectorAll("[data-extra-child-row]").forEach((row) => {
    if (row.dataset.fullChildFields === "1") return;
    row.dataset.fullChildFields = "1";
    row.style.gridTemplateColumns = "repeat(2, minmax(0, 1fr))";
    row.style.gap = "10px";

    const html = `
      <div class="field"><label>تاريخ الميلاد هجري</label><input data-extra-child-hijri type="text" dir="ltr" lang="en" inputmode="numeric" placeholder="1445-09-01"></div>
      <div class="field"><label>تاريخ الميلاد ميلادي</label><input data-extra-child-greg type="date" dir="ltr" lang="en"></div>
      ...
      <div class="field"><label>المدينة</label><input data-extra-child-city type="text" placeholder="المدينة"></div>
      <div class="field"><label>الحي / القرية</label><input data-extra-child-area type="text" placeholder="حي كذا أو قرية كذا"></div>
      <label style="display:flex;align-items:center;gap:8px;margin-top:28px;"><input data-extra-child-deceased type="checkbox"> <span>متوفى</span></label>
    `;
    row.insertAdjacentHTML("beforeend", html);
  });
}

function collectDelegateChildRowsForSave() {
  const rows = [{
    name: childName ? childName.value : "",
    order: childBirthOrder ? childBirthOrder.value : "",
    hijri: childBirthHijri ? childBirthHijri.value : "",
    greg: childBirthGreg ? childBirthGreg.value : "",
    age: "",
  ...
  document.querySelectorAll("[data-extra-child-row]").forEach((row) => {
    ...
    rows.push({
      name: get("[data-extra-child-name]") ? get("[data-extra-child-name]").value : "",
      order: get("[data-extra-child-order]") ? get("[data-extra-child-order]").value : "",
      hijri: get("[data-extra-child-hijri]") ? get("[data-extra-child-hijri]").value : "",
      greg: get("[data-extra-child-greg]") ? get("[data-extra-child-greg]").value : "",
      age: get("[data-extra-child-age]") ? get("[data-extra-child-age]").value : "",
      city: get("[data-extra-child-city]") ? get("[data-extra-child-city]").value : "",
      area: get("[data-extra-child-area]") ? get("[data-extra-child-area]").value : "",
      deceased: !!(get("[data-extra-child-deceased]") && get("[data-extra-child-deceased]").checked),
      main: false
    });
  });

  return rows.filter((r) => normalizePersonName(r.name));
}
...
setTimeout(enhanceDelegateExtraChildRows, 300);
document.addEventListener("click", () => setTimeout(enhanceDelegateExtraChildRows, 50));

parentSelect.addEventListener("change", () =>{ closeEditChild(); closeWifeChildrenManager(); renderChildrenForParent(parentSelect.value); loadWivesForSelectedParent(); }); addChildBtn.addEventListener("click", async () =>{ if (!state.branch) { setChildAlert("error", "يلزم تسجيل دخول المندوب أولاً."); return; } const selectedParentName = resolveSelectedParentId(normalizePersonName(parentSelect.value), state.branch); const rawName = normalizePersonName(childName.value); const deceased = !!(childDeceased && childDeceased.checked); const hijriInput = deceased ? "" : (childBirthHijri ? String(childBirthHijri.value || "").trim() : ""); const gregInput = deceased ? "" : (childBirthGreg ? String(childBirthGreg.value || "").trim() : ""); const hijriNorm = hijriInput ? normalizeHijriDateISO(hijriInput) : ""; const gregNorm = gregInput ? normalizeGregorianDateISO(gregInput) : ""; if (hijriInput && !hijriNorm) { setChildAlert("error", "تاريخ الميلاد (هجري) غير صحيح. الصيغة: YYYY-MM-DD"); return; } if (gregInput && !gregNorm) { setChildAlert("error", "تاريخ الميلاد (ميلادي) غير صحيح."); return; } let finalHijri = hijriNorm; let finalGreg = gregNorm; if (finalHijri && !finalGreg) finalGreg = hijriToGregorianISO(finalHijri); if (finalGreg && !finalHijri) finalHijri = gregorianToHijriISO(finalGreg); if (finalHijri && !finalGreg) { setChildAlert("error", "تعذر تحويل التاريخ الهجري إلى ميلادي."); return; } if (finalGreg && !finalHijri) { setChildAlert("error", "تعذر تحويل التاريخ الميلادي إلى هجري."); return; } const birthYear = finalHijri ? normalizeBirthYear(finalHijri.slice(0, 4)) : null; const birthOrderRaw = childBirthOrder ? normalizeArabicDigitsToLatin(String(childBirthOrder.value || "").trim()) : ""; const birthOrder = birthOrderRaw ? parseInt(birthOrderRaw, 10) : null; if (birthOrderRaw && (!birthOrder || birthOrder< 1 || String(birthOrder) !== birthOrderRaw)) { setChildAlert("error", "ترتيب الميلاد يجب أن يكون رقمًا صحيحًا يبدأ من 1."); return; } const city = deceased ? "" : normalizePersonName(childCity.value); const area = deceased ? "" : normalizePersonName(childArea.value); if (!rawName) { setChildAlert("error", "يرجى إدخال اسم الابن."); return; } const sb = getالخدمةClient(); if (!sb) { setChildAlert("error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } const buildChildId = (parentId, baseName) =>{ const p = normalizePersonName(parentId || ""); const b = normalizePersonName(baseName || ""); if (!p || !b) return ""; return p + "/" + b; }; const tokens = tokenizeLineageInput(rawName); const lineagePlan = buildLineagePlanFromTokens(tokens, state.branch, selectedParentName); const baseNames = getAllBaseNames(); const nowIso = new Date().toISOString();
 const delegateRows = collectDelegateChildRowsForSave();
 if (delegateRows.length > 1 && selectedParentName && !(lineagePlan && lineagePlan.anchorParent && Array.isArray(lineagePlan.chain) && lineagePlan.chain.length)) {
   const parentName = selectedParentName;
   let inserted = 0;
   let skipped = 0;

   for (let i = 0; i < delegateRows.length; i += 1) {
     const item = delegateRows[i];
     const base = normalizePersonBaseName(item.name);
     if (!base) continue;
     ...
     const rowDeceased = !!item.deceased;
     const rowHijriInput = rowDeceased ? "" : String(item.hijri || "").trim();
     const rowGregInput = rowDeceased ? "" : String(item.greg || "").trim();
     const rowHijriNorm = rowHijriInput ? normalizeHijriDateISO(rowHijriInput) : "";
     const rowGregNorm = rowGregInput ? normalizeGregorianDateISO(rowGregInput) : "";

     if (rowHijriInput && !rowHijriNorm) {
       setChildAlert("error", "تاريخ الميلاد الهجري غير صحيح للابن: " + base);
       return;
     }
     if (rowGregInput && !rowGregNorm) {
       setChildAlert("error", "تاريخ الميلاد الميلادي غير صحيح للابن: " + base);
       return;
     }

     let rowFinalHijri = rowHijriNorm;
     let rowFinalGreg = rowGregNorm;
     if (rowFinalHijri && !rowFinalGreg) rowFinalGreg = hijriToGregorianISO(rowFinalHijri);
     if (rowFinalGreg && !rowFinalHijri) rowFinalHijri = gregorianToHijriISO(rowFinalGreg);

     const rowBirthYear = rowFinalHijri
       ? normalizeBirthYear(rowFinalHijri.slice(0, 4))
       : delegateBirthYearFromAge(item.age);

     const rowOrderRaw = item.order ? normalizeArabicDigitsToLatin(String(item.order).trim()) : "";
     const rowOrder = rowOrderRaw ? parseInt(rowOrderRaw, 10) : null;
     if (rowOrderRaw && (!rowOrder || rowOrder < 1 || String(rowOrder) !== rowOrderRaw)) {
       setChildAlert("error", "ترتيب الميلاد غير صحيح للابن: " + base);
       return;
     }
     ...
     const saveRow = {
       ...
       child_name: childId,
       birth_date_g: rowFinalGreg || null,
       birth_date_h: rowFinalHijri || null,
       birth_year: rowBirthYear,
       birth_order: rowOrder,
       city: rowDeceased ? null : (normalizePersonName(item.city) || null),
       area: rowDeceased ? null : (normalizePersonName(item.area) || null),
       is_deceased: rowDeceased,
       created_at: nowIso
     };

     const insertRes = await rpcInsertTreeChildRow(sb, saveRow);
     if (!insertRes.ok) {
       setChildAlert("error", formatTreeChildrenDbError(insertRes.error || { message: "unknown error" }, "save"));
       return;
     }
     inserted += 1;
   }
   ...
   if (childBirthOrder) childBirthOrder.value = "";
   if (childCity) childCity.value = "";
   if (childArea) childArea.value = "";
   if (childDeceased) childDeceased.checked = false;
   document.querySelectorAll("[data-extra-child-row]").forEach((row) => row.remove());

   const reloadRes = await loadChildrenForBranch(state.branch, { applyToState: true });
   if (!reloadRes.ok) {
     setChildAlert("success", "تم حفظ الأبناء، لكن تعذر تحديث العرض الآن.");
     return;
   }
   renderParentsForBranch(state.branch);
   selectParentForAdding(parentName);
   renderChildrenForParent(parentName);
   setChildAlert("success", "تم حفظ الأبناء. تمت إضافة: " + inserted + "، وتجاهل المكرر: " + skipped);
   return;
 }
 const getSiblingPartsFromRawInput = (input, selectedParentId, plan) =>{ const v = normalizePersonName(input || ""); if (!v) return []; if (!selectedParentId) return []; if (plan && plan.anchorParent && Array.isArray(plan.chain) && plan.chain.length) return []; const parts = []; const pushAll = (arr) =>{ (Array.isArray(arr) ? arr : []).forEach((p) =>{ const n = normalizePersonName(p); if (n) parts.push(n); }); }; if (/[&,،]/.test(v) || v.includes("\n")) { pushAll(v.split(/[&,،\n]/g)); } else if (/\s+و\s+/.test(v)) { pushAll(v.split(/\s+و\s+/g)); } else if (/\s+/.test(v) && !/\b(بن|ابن|بنت)\b/.test(v)) { pushAll(v.split(/\s+/g)); } const uniq = []; const seen = new Set(); parts.forEach((p) =>{ const key = normalizePersonName(p); if (!key || seen.has(key)) return; seen.add(key); uniq.push(key); }); return uniq; }; const siblingParts = getSiblingPartsFromRawInput(rawName, selectedParentName, lineagePlan); if (siblingParts.length >1) { if (!selectedParentName) { setChildAlert("error", "يرجى اختيار الأب أولاً لإضافة عدة أسماء كإخوة."); return; } const parentName = selectedParentName; let inserted = 0; let skipped = 0; for (let i = 0; i< siblingParts.length; i++) { const part = siblingParts[i]; const base = normalizePersonBaseName(part); if (!base) continue; if (normalizePersonBaseName(base) === normalizePersonBaseName(parentName)) { setChildAlert("error", "لا يمكن أن يكون اسم الابن مطابقًا لاسم الأب."); return; } if (findChildNameByBase(parentName, base)) { skipped += 1; continue; } const childId = buildChildId(parentName, base); if (!childId) continue; const row = { branch_key: state.branch, parent_name: parentName, child_name: childId, birth_date_g: finalGreg || null, birth_date_h: finalHijri || null, birth_year: birthYear, birth_order: birthOrder == null ? null : birthOrder + i, city: city || null, area: area || null, is_deceased: deceased, created_at: nowIso }; const insertRes = await rpcInsertTreeChildRow(sb, row); if (!insertRes.ok) { const err = insertRes.error || { message: "unknown error" }; if (isRpcMissingError(err)) { setChildAlert( "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } setChildAlert("error", formatTreeChildrenDbError(err, "save")); return; } if (!state.children[parentName]) state.children[parentName] = []; state.children[parentName].push({ name: childId, year: birthYear ? String(birthYear) : "", order: birthOrder == null ? "" : String(birthOrder + i), gdate: finalGreg || "", hdate: finalHijri || "", city: city || "", area: area || "", deceased }); baseNames.add(base); inserted += 1; } childName.value = ""; if (childBirthHijri) childBirthHijri.value = ""; if (childBirthGreg) childBirthGreg.value = ""; if (childBirthOrder) childBirthOrder.value = ""; childCity.value = ""; childArea.value = ""; if (childPhone) childPhone.value = ""; if (childDeceased) childDeceased.checked = false; applyAddDeceasedUiMode(); const reloadRes = await loadChildrenForBranch(state.branch, { applyToState: true }); if (!reloadRes.ok) { setChildAlert( "success", "تم حفظ الأسماء في قاعدة البيانات، لكن تعذر تحديث العرض الآن. " + formatTreeChildrenDbError(reloadRes.error, "load") ); return; } renderParentsForBranch(state.branch); selectParentForAdding(parentName); setChildAlert("success", "تم حفظ الأسماء كإخوة. تمت إضافة: " + String(inserted) + "، وتجاهل المكرر: " + String(skipped)); renderChildrenForParent(parentName); return; } if (lineagePlan && lineagePlan.anchorParent && lineagePlan.chain && lineagePlan.chain.length) { let currentParent = normalizePersonName(lineagePlan.anchorParent); let inserted = 0; let skipped = 0; let youngestFinal = ""; for (let i = 0; i< lineagePlan.chain.length; i++) { const desiredChild = normalizePersonName(lineagePlan.chain[i]); const desiredBase = normalizePersonBaseName(desiredChild); if (!desiredBase) continue; if (desiredBase === normalizePersonBaseName(currentParent)) { setChildAlert("error", "لا يمكن أن يكون اسم الابن مطابقًا لاسم الأب."); return; } const isYoungest = i === lineagePlan.chain.length - 1; const finalChild = normalizePersonBaseName(desiredChild); const tokensCheck = tokenizeLineageInput(finalChild); if (isYoungest && tokensCheck.length !== 1) { setChildAlert("error", "ممنوع تسجيل الاسم الأخير بأكثر من كلمة. اكتب اسم الابن فقط."); return; } const finalChildBase = normalizePersonBaseName(finalChild); if (!finalChild || !finalChildBase) continue; const existingChildName = findChildNameByBase(currentParent, finalChildBase); if (existingChildName) { skipped += 1; currentParent = existingChildName; youngestFinal = existingChildName; continue; } const childId = buildChildId(currentParent, finalChildBase); if (!childId) continue; const row = { branch_key: state.branch, parent_name: currentParent, child_name: childId, birth_date_g: isYoungest ? (finalGreg || null) : null, birth_date_h: isYoungest ? (finalHijri || null) : null, birth_year: isYoungest ? birthYear : null, birth_order: isYoungest ? birthOrder : null, city: isYoungest ? (city || null) : null, area: isYoungest ? (area || null) : null, is_deceased: isYoungest ? deceased : null, created_at: nowIso }; const insertRes = await rpcInsertTreeChildRow(sb, row); if (!insertRes.ok) { const err = insertRes.error || { message: "unknown error" }; if (isRpcMissingError(err)) { setChildAlert( "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } setChildAlert("error", formatTreeChildrenDbError(err, "save")); return; } if (!state.children[currentParent]) state.children[currentParent] = []; state.children[currentParent].push({ name: childId, year: isYoungest && birthYear ? String(birthYear) : "", order: isYoungest && birthOrder ? String(birthOrder) : "", gdate: isYoungest ? (finalGreg || "") : "", hdate: isYoungest ? (finalHijri || "") : "", city: isYoungest ? (city || "") : "", area: isYoungest ? (area || "") : "", deceased: isYoungest ? deceased : false }); baseNames.add(normalizePersonBaseName(finalChild)); inserted += 1; currentParent = childId; youngestFinal = childId; } childName.value = ""; if (childBirthHijri) childBirthHijri.value = ""; if (childBirthGreg) childBirthGreg.value = ""; if (childBirthOrder) childBirthOrder.value = ""; childCity.value = ""; childArea.value = ""; if (childDeceased) childDeceased.checked = false; const reloadRes = await loadChildrenForBranch(state.branch, { applyToState: true }); if (!reloadRes.ok) { setChildAlert( "success", "تم حفظ السلسلة في قاعدة البيانات، لكن تعذر تحديث العرض الآن. " + formatTreeChildrenDbError(reloadRes.error, "load") ); return; } renderParentsForBranch(state.branch); if (youngestFinal) selectParentForAdding(youngestFinal); setChildAlert( "success", "تم حفظ السلسلة. تمت إضافة: " + String(inserted) + "، وتجاهل المكرر: " + String(skipped) ); if (!youngestFinal) { if (selectedParentName) renderChildrenForParent(selectedParentName); else renderChildrenForParent(lineagePlan.anchorParent); } return; } if (!selectedParentName) { setChildAlert("error", "يرجى اختيار الأب أولاً أو اكتب الاسم كسلسلة تنتهي باسم الفرع."); return; } const parentName = selectedParentName; if (normalizePersonBaseName(rawName) === normalizePersonBaseName(parentName)) { setChildAlert("error", "لا يمكن أن يكون اسم الابن مطابقًا لاسم الأب."); return; } const tokensCheck = tokenizeLineageInput(rawName); if (tokensCheck.length !== 1) { setChildAlert("error", "ممنوع تسجيل الاسم الأخير بأكثر من كلمة. اكتب اسم الابن فقط."); return; } const inputBase = normalizePersonBaseName(rawName); if (findChildNameByBase(parentName, inputBase)) { setChildAlert("error", "اسم الابن مسجل مسبقًا لهذا الأب."); return; } const finalName = normalizePersonBaseName(rawName); if (normalizePersonBaseName(finalName) === normalizePersonBaseName(parentName)) { setChildAlert("error", "لا يمكن أن يكون اسم الابن مطابقًا لاسم الأب."); return; } const childId = buildChildId(parentName, finalName); if (!childId) { setChildAlert("error", "تعذر حفظ الاسم بسبب خطأ في بناء المعرف."); return; } const row = { branch_key: state.branch, parent_name: parentName, child_name: childId, birth_date_g: finalGreg || null, birth_date_h: finalHijri || null, birth_year: birthYear, birth_order: birthOrder, city: city || null, area: area || null, is_deceased: deceased, created_at: nowIso }; const insertRes = await rpcInsertTreeChildRow(sb, row); if (!insertRes.ok) { const err = insertRes.error || { message: "unknown error" }; if (isRpcMissingError(err)) { setChildAlert( "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } setChildAlert("error", formatTreeChildrenDbError(insertRes.error, "save")); return; } const memberPhoneForChild = childPhone ? normalizeMemberPhoneForDelegate(childPhone.value) : "";
 const memberProfileRes = await saveDelegateMemberProfile(sb, memberPhoneForChild, state.branch, childId, "");
 if (!memberProfileRes.ok) {
   setChildAlert("error", "تم حفظ الابن لكن تعذر حفظ رقم الجوال: " + ((memberProfileRes.error && memberProfileRes.error.message) || "خطأ غير معروف"));
   return;
 }
 const motherLinkRes = await linkChildToSelectedSpouse(childId); if (!motherLinkRes.ok) { setChildAlert("error", "تم حفظ الابن لكن تعذر ربط الأم: " + ((motherLinkRes.error && motherLinkRes.error.message) || "خطأ غير معروف")); return; } childName.value = ""; if (childBirthHijri) childBirthHijri.value = ""; if (childBirthGreg) childBirthGreg.value = ""; if (childBirthOrder) childBirthOrder.value = ""; childCity.value = ""; childArea.value = ""; if (childDeceased) childDeceased.checked = false; applyAddDeceasedUiMode(); const reloadRes = await loadChildrenForBranch(state.branch, { applyToState: true }); if (!reloadRes.ok) { setChildAlert( "success", "تم حفظ بيانات الابن في قاعدة البيانات، لكن تعذر تحديث العرض الآن. " + formatTreeChildrenDbError(reloadRes.error, "load") ); return; } renderParentsForBranch(state.branch); selectParentForAdding(childId); setChildAlert("success", "تم حفظ بيانات الابن في قاعدة البيانات: " + finalName); }); function syncEditBirthFromHijri() { if (isSyncingBirthDates) return; if (!editChildHijri || !editChildGreg) return; const hijriValue = String(editChildHijri.value || "").trim(); if (!hijriValue) return; const hijriISO = normalizeHijriDateISO(hijriValue); if (!hijriISO) return; const gregISO = hijriToGregorianISO(hijriISO); if (!gregISO) return; isSyncingBirthDates = true; editChildHijri.value = hijriISO; editChildGreg.value = gregISO; isSyncingBirthDates = false; } function syncEditBirthFromGreg() { if (isSyncingBirthDates) return; if (!editChildHijri || !editChildGreg) return; const gregISO = normalizeGregorianDateISO(String(editChildGreg.value || "").trim()); if (!gregISO) return; const hijriISO = gregorianToHijriISO(gregISO); if (!hijriISO) return; isSyncingBirthDates = true; editChildGreg.value = gregISO; editChildHijri.value = hijriISO; isSyncingBirthDates = false; } if (editChildHijri) { editChildHijri.addEventListener("input", () =>{ syncEditBirthFromHijri(); }); editChildHijri.addEventListener("blur", () =>{ syncEditBirthFromHijri(); }); } if (editChildGreg) { editChildGreg.addEventListener("change", () =>{ syncEditBirthFromGreg(); }); } if (childDeceased) { childDeceased.addEventListener("change", () =>{ applyAddDeceasedUiMode(); }); applyAddDeceasedUiMode(); } if (editChildDeceased) { editChildDeceased.addEventListener("change", () =>{ applyEditDeceasedUiMode(); }); } if (cancelEditBtn) { cancelEditBtn.addEventListener("click", () =>{ closeEditChild(); }); } if (saveEditBtn) { saveEditBtn.addEventListener("click", async () =>{ if (!editingChildRef || !editingChildRef.childId || !editingChildRef.parentId) return; if (!state.branch) { setEditChildAlert("error", "يلزم تسجيل دخول المندوب أولاً."); return; } const deceased = !!(editChildDeceased && editChildDeceased.checked); const hijriInput = deceased ? "" : (editChildHijri ? String(editChildHijri.value || "").trim() : ""); const gregInput = deceased ? "" : (editChildGreg ? String(editChildGreg.value || "").trim() : ""); const hijriNorm = hijriInput ? normalizeHijriDateISO(hijriInput) : ""; const gregNorm = gregInput ? normalizeGregorianDateISO(gregInput) : ""; if (hijriInput && !hijriNorm) { setEditChildAlert("error", "تاريخ الميلاد (هجري) غير صحيح. الصيغة: YYYY-MM-DD"); return; } if (gregInput && !gregNorm) { setEditChildAlert("error", "تاريخ الميلاد (ميلادي) غير صحيح."); return; } let finalHijri = hijriNorm; let finalGreg = gregNorm; if (finalHijri && !finalGreg) finalGreg = hijriToGregorianISO(finalHijri); if (finalGreg && !finalHijri) finalHijri = gregorianToHijriISO(finalGreg); if (finalHijri && !finalGreg) { setEditChildAlert("error", "تعذر تحويل التاريخ الهجري إلى ميلادي."); return; } if (finalGreg && !finalHijri) { setEditChildAlert("error", "تعذر تحويل التاريخ الميلادي إلى هجري."); return; } const birthYear = finalHijri ? normalizeBirthYear(finalHijri.slice(0, 4)) : null; const birthOrderRaw = editChildBirthOrder ? normalizeArabicDigitsToLatin(String(editChildBirthOrder.value || "").trim()) : ""; const birthOrder = birthOrderRaw ? parseInt(birthOrderRaw, 10) : null; if (birthOrderRaw && (!birthOrder || birthOrder< 1 || String(birthOrder) !== birthOrderRaw)) { setEditChildAlert("error", "ترتيب الميلاد يجب أن يكون رقمًا صحيحًا يبدأ من 1."); return; } const city = deceased ? "" : (editChildCity ? normalizePersonName(editChildCity.value) : ""); const area = deceased ? "" : (editChildArea ? normalizePersonName(editChildArea.value) : ""); const sb = getالخدمةClient(); if (!sb) { setEditChildAlert("error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } const patch = { birth_date_g: finalGreg || null, birth_date_h: finalHijri || null, birth_year: birthYear, birth_order: birthOrder, city: city || null, area: area || null, is_deceased: deceased }; const res = await rpcUpdateTreeChildRow( sb, state.branch, editingChildRef.parentId, editingChildRef.childId, patch, editingChildRef.personId ); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { setEditChildAlert( "error", "تعذر تنفيذ التعديل حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } setEditChildAlert("error", formatTreeChildrenDbError(err, "update")); return; } const editPhoneValue = editChildPhone ? normalizeMemberPhoneForDelegate(editChildPhone.value) : "";
 const memberProfileEditRes = await saveDelegateMemberProfile(sb, editPhoneValue, state.branch, editingChildRef.childId, editingChildRef.personId);
 if (!memberProfileEditRes.ok) {
   setEditChildAlert("error", "تم حفظ التعديل لكن تعذر حفظ رقم الجوال: " + ((memberProfileEditRes.error && memberProfileEditRes.error.message) || "خطأ غير معروف"));
   return;
 }
 const applyPatchToState = () =>{ const parentId = normalizePersonName(editingChildRef.parentId); const childId = normalizePersonName(editingChildRef.childId); if (!parentId || !childId) return false; const updateChild = (p) =>{ const key = normalizePersonName(p); if (!key) return false; const list = state.children[key]; if (!Array.isArray(list) || !list.length) return false; const idx = list.findIndex((c) =>normalizePersonName(c && c.name ? c.name : "") === childId); if (idx< 0) return false; const prev = list[idx] || {}; list[idx] = { ...prev, year: birthYear ? String(birthYear) : "", order: birthOrder ? String(birthOrder) : "", gdate: finalGreg || "", hdate: finalHijri || "", city: city || "", area: area || "", deceased }; return true; }; if (updateChild(parentId)) return true; const parents = Object.keys(state.children || {}); for (let i = 0; i< parents.length; i++) { if (updateChild(parents[i])) return true; } return false; }; applyPatchToState(); const reloadRes = await loadChildrenForBranch(state.branch, { applyToState: true }); if (!reloadRes.ok) { renderParentsForBranch(state.branch); ensureParentOption(editingChildRef.parentId); parentSelect.value = editingChildRef.parentId; renderChildrenForParent(editingChildRef.parentId); closeEditChild(); setChildAlert( "success", "تم حفظ التعديل. تعذر تحديث البيانات من قاعدة البيانات الآن، لكن تم تحديث العرض محلياً." ); return; } renderParentsForBranch(state.branch); ensureParentOption(editingChildRef.parentId); parentSelect.value = editingChildRef.parentId; renderChildrenForParent(editingChildRef.parentId); closeEditChild(); setChildAlert( "success", res.degraded ? "تم حفظ التعديل، لكن تعذر حفظ حالة متوفى في الخدمة لأن عمود الوفاة غير متاح." : "تم حفظ التعديل." ); }); } const EVENTS_REFRESH_KEY = "alzidan_events_refresh_v1"; function touchEventsRefresh() { try { localStorage.setItem(EVENTS_REFRESH_KEY, String(Date.now())); } catch (e) {} } 
function getDelegateHappyMediaInputs() {
  return {
    imageUrl: document.getElementById("happy-image-url"),
    imageFile: document.getElementById("happy-image-file"),
    videoUrl: document.getElementById("happy-video-url"),
    videoFile: document.getElementById("happy-video-file")
  };
}
function delegateFileExtFromName(name, fallback) {
...
function resetHappyMediaInputs() {
  const el = getDelegateHappyMediaInputs();
...
function fillHappyMediaInputsFromRow(row) {
  const d = parseDelegateHappyDetails(row || {});
  const el = getDelegateHappyMediaInputs();
  if (el.imageUrl) el.imageUrl.value = d.imageUrl || d.image_url || d.photoUrl || d.photo_url || "";
  if (el.videoUrl) el.videoUrl.value = d.videoUrl || d.video_url || "";
  if (el.imageFile) el.imageFile.value = "";
  if (el.videoFile) el.videoFile.value = "";
}
async function getHappyMediaValues(sb, oldRow) {
  const el = getDelegateHappyMediaInputs();
  const old = parseDelegateHappyDetails(oldRow || {});
  let imageUrl = el.imageUrl && el.imageUrl.value ? el.imageUrl.value.trim() : (old.imageUrl || old.image_url || old.photoUrl || old.photo_url || "");
  let videoUrl = el.videoUrl && el.videoUrl.value ? el.videoUrl.value.trim() : (old.videoUrl || old.video_url || "");
  if (el.imageFile && el.imageFile.files && el.imageFile.files[0]) imageUrl = await uploadDelegateEventMedia(sb, el.imageFile.files[0], "image");
  if (el.videoFile && el.videoFile.files && el.videoFile.files[0]) videoUrl = await uploadDelegateEventMedia(sb, el.videoFile.files[0], "video");
  return { imageUrl, videoUrl };
}
function appendDelegateEventMedia(rowEl, eventRow) {
  const d = parseDelegateHappyDetails(eventRow || {});
  const imageUrl = String(d.imageUrl || d.image_url || d.photoUrl || d.photo_url || "").trim();
  const videoUrl = String(d.videoUrl || d.video_url || "").trim();
  if (!imageUrl && !videoUrl) return;
  const wrap = document.createElement("div");
  wrap.className = "delegate-media-preview";
  if (imageUrl) {
    const img = document.createElement("img");
    img.alt = "صورة المناسبة";
    img.loading = "lazy";
    img.src = imageUrl;
 ...
 function resetHappyForm() { editingHappy = null; if (happyType) happyType.value = ""; if (happyPerson) happyPerson.value = ""; if (happyDate) happyDate.value = todayGregorianISO(); if (happyVisibilityDays) happyVisibilityDays.value = "7"; if (happyDetails) happyDetails.value = ""; resetHappyMediaInputs(); if (saveHappyBtn) saveHappyBtn.textContent = "إضافة"; if (cancelHappyBtn) cancelHappyBtn.style.display = "none"; } function resetSickForm() { editingSick = null; if (sickType) sickType.value = ""; if (sickPerson) sickPerson.value = ""; if (sickDate) sickDate.value = todayGregorianISO(); if (sickVisibilityDays) sickVisibilityDays.value = "7"; if (sickPlace) sickPlace.value = "hospital"; if (sickHospitalName) sickHospitalName.value = ""; if (sickHospitalDept) sickHospitalDept.value = ""; if (sickHomeCity) sickHomeCity.value = ""; if (sickHomeArea) sickHomeArea.value = ""; if (sickContactMethod) sickContactMethod.value = "visit"; if (sickVisitDateFrom) sickVisitDateFrom.value = ""; if (sickVisitDateTo) sickVisitDateTo.value = ""; if (sickVisitTimeFrom) sickVisitTimeFrom.value = ""; if (sickVisitTimeTo) sickVisitTimeTo.value = ""; if (sickVisitTimeFromHour) sickVisitTimeFromHour.value = ""; if (sickVisitTimeFromMinute) sickVisitTimeFromMinute.value = ""; if (sickVisitTimeFromPeriod) sickVisitTimeFromPeriod.value = ""; if (sickVisitTimeToHour) sickVisitTimeToHour.value = ""; if (sickVisitTimeToMinute) sickVisitTimeToMinute.value = ""; if (sickVisitTimeToPeriod) sickVisitTimeToPeriod.value = ""; if (sickContactPhone) sickContactPhone.value = ""; if (sickNotes) sickNotes.value = ""; if (saveSickBtn) saveSickBtn.textContent = "إضافة"; if (cancelSickBtn) cancelSickBtn.style.display = "none"; updateSickPlaceVisibility(); updateSickContactModeVisibility(); } function resetDeathForm() { editingDeath = null; if (deathPerson) deathPerson.value = ""; if (deathDate) deathDate.value = todayGregorianISO(); if (deathVisibilityDays) deathVisibilityDays.value = "7"; if (prayerPlace) prayerPlace.value = ""; if (prayerTime) prayerTime.value = ""; if (burialPlace) burialPlace.value = ""; if (burialTime) burialTime.value = ""; if (condolencePlace) condolencePlace.value = ""; if (condolenceTime) condolenceTime.value = ""; if (condolencePhone1) condolencePhone1.value = ""; if (condolencePhone2) condolencePhone2.value = ""; if (deathNotes) deathNotes.value = ""; if (saveDeathBtn) saveDeathBtn.textContent = "نشر"; if (cancelDeathBtn) cancelDeathBtn.style.display = "none"; } function resolveEventDateInputValue(row) { const raw = row && row.event_date != null ? String(row.event_date).trim() : ""; const iso = /^(\d{4})-(\d{1,2})-(\d{1,2})/.exec(raw); if (iso) { const y = iso[1].padStart(4, "0"); const m = iso[2].padStart(2, "0"); const d = iso[3].padStart(2, "0"); const year = parseInt(y, 10); if (year >= 1200 && year<= 1700) { const gregorian = hijriToGregorianISO(y + "-" + m + "-" + d); if (gregorian) return gregorian; } return y + "-" + m + "-" + d; } const labelRaw = row && row.date_label != null ? String(row.date_label).trim() : ""; let s = labelRaw; for (let i = 0; i< 3; i++) { const m = /^\s*[\(（]\s*(.*?)\s*[\)）]\s*$/.exec(s); if (!m) break; s = String(m[1] || "").trim(); if (!s) break; } const labelIso = /^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(s); if (labelIso) { const y = labelIso[1].padStart(4, "0"); const m = labelIso[2].padStart(2, "0"); const d = labelIso[3].padStart(2, "0"); const year = parseInt(y, 10); if (year >= 1200 && year<= 1700) { const g = hijriToGregorianISO(y + "-" + m + "-" + d); if (g) return g; } if (year >= 1900 && year<= 2100) return y + "-" + m + "-" + d; } const slash = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(s); if (slash) { const d = slash[1].padStart(2, "0"); const m = slash[2].padStart(2, "0"); const y = slash[3]; const year = parseInt(y, 10); if (year >= 1200 && year<= 1700) { const g = hijriToGregorianISO(y + "-" + m + "-" + d); if (g) return g; } if (year >= 1900 && year<= 2100) return y + "-" + m + "-" + d; } return todayGregorianISO(); } function startEditHappy(row) { editingHappy = row || null; if (happySection) happySection.open = true; if (happyType) happyType.value = String(row && row.type ? row.type : ""); if (happyPerson) happyPerson.value = normalizePersonName(row && row.person ? row.person : ""); if (happyDate) happyDate.value = resolveEventDateInputValue(row || {}); if (happyVisibilityDays) happyVisibilityDays.value = String(getEventVisibilityDays(row || {})); if (happyDetails) happyDetails.value = getHappyDetailsText(row || {}); fillHappyMediaInputsFromRow(row || {}); if (saveHappyBtn) saveHappyBtn.textContent = "حفظ التعديل"; if (cancelHappyBtn) cancelHappyBtn.style.display = "inline-flex"; } function startEditSick(row) { editingSick = row || null; if (sickSection) sickSection.open = true; if (sickType) sickType.value = String(row && row.type ? row.type : ""); if (sickPerson) sickPerson.value = normalizePersonName(row && row.person ? row.person : ""); if (sickDate) sickDate.value = resolveEventDateInputValue(row || {}); if (sickVisibilityDays) sickVisibilityDays.value = String(getEventVisibilityDays(row || {})); const health = parseHealthDetails(row || {}); if (sickPlace) sickPlace.value = health.place === "home" ? "home" : "hospital"; if (sickHomeCity) sickHomeCity.value = health.homeCity || ""; if (sickHomeArea) sickHomeArea.value = health.homeArea || ""; if (sickNotes) sickNotes.value = health.notes || ""; const hospitalNameValue = String(row && (row.hospital_name || row.hospitalName) ? (row.hospital_name || row.hospitalName) : health.hospitalName || "").trim(); const hospitalDeptValue = String(row && (row.hospital_dept || row.hospitalDept) ? (row.hospital_dept || row.hospitalDept) : health.hospitalDept || "").trim(); if (sickHospitalName) sickHospitalName.value = hospitalNameValue; if (sickHospitalDept) sickHospitalDept.value = hospitalDeptValue; if (sickContactMethod) sickContactMethod.value = String(row && row.contact_method ? row.contact_method : "visit"); if (sickContactPhone) sickContactPhone.value = String(row && row.contact_phone ? row.contact_phone : ""); if (sickVisitDateFrom) sickVisitDateFrom.value = String(row && row.visit_date_from ? row.visit_date_from : ""); if (sickVisitDateTo) sickVisitDateTo.value = String(row && row.visit_date_to ? row.visit_date_to : ""); if (sickVisitTimeFrom) sickVisitTimeFrom.value = String(row && row.visit_time_from ? row.visit_time_from : ""); if (sickVisitTimeTo) sickVisitTimeTo.value = String(row && row.visit_time_to ? row.visit_time_to : ""); setVisitTimeSelectorsFromValue(sickVisitTimeFromHour, sickVisitTimeFromMinute, sickVisitTimeFromPeriod, sickVisitTimeFrom && sickVisitTimeFrom.value ? sickVisitTimeFrom.value : ""); setVisitTimeSelectorsFromValue(sickVisitTimeToHour, sickVisitTimeToMinute, sickVisitTimeToPeriod, sickVisitTimeTo && sickVisitTimeTo.value ? sickVisitTimeTo.value : ""); updateSickPlaceVisibility(); updateSickContactModeVisibility(); if (saveSickBtn) saveSickBtn.textContent = "حفظ التعديل"; if (cancelSickBtn) cancelSickBtn.style.display = "inline-flex"; } function startEditDeath(row) { editingDeath = row || null; if (deathSection) deathSection.open = true; if (deathPerson) deathPerson.value = normalizePersonName(row && row.person ? row.person : ""); if (deathDate) deathDate.value = resolveEventDateInputValue(row || {}); if (deathVisibilityDays) deathVisibilityDays.value = String(getEventVisibilityDays(row || {})); const d = parseDeathDetails(row || {}); if (prayerPlace) prayerPlace.value = d.prayerPlace || ""; if (prayerTime) prayerTime.value = d.prayerTime || ""; if (burialPlace) burialPlace.value = d.burialPlace || ""; if (burialTime) burialTime.value = d.burialTime || ""; if (condolencePlace) condolencePlace.value = d.condolencePlace || ""; if (condolenceTime) condolenceTime.value = d.condolenceTime || ""; if (condolencePhone1) condolencePhone1.value = d.phones && d.phones[0] ? d.phones[0] : ""; if (condolencePhone2) condolencePhone2.value = d.phones && d.phones[1] ? d.phones[1] : ""; if (deathNotes) deathNotes.value = d.notes || ""; if (saveDeathBtn) saveDeathBtn.textContent = "حفظ التعديل"; if (cancelDeathBtn) cancelDeathBtn.style.display = "inline-flex"; } async function deleteEventRow(row, opts) { const options = opts || {}; const pk = getEventPk(row); if (!pk) return; const personName = normalizePersonName(row && row.person ? row.person : ""); const confirmName = personName || "حذف"; const ok = await confirmTypedText(confirmName, { title: "تأكيد حذف الخبر", body: "لتأكيد الحذف اكتب الاسم التالي بالضبط:", confirmLabel: "تأكيد الحذف", cancelLabel: "إلغاء" }); if (!ok) return; const sb = getالخدمةClient(); const alertEl = options.kind === "death" ? deathAlert : options.kind === "sick" ? sickAlert : happyAlert; if (!sb) { showAlertAndFocus(alertEl, "error", "تعذر الحذف لأن الربط غير مُعد."); return; } const res = await rpcDeleteFamilyEventRow(sb, pk); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { showAlertAndFocus( alertEl, "error", "تعذر الحذف حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } const msg = String(err && err.message ? err.message : ""); if (msg.toLowerCase().includes("not allowed")) { showAlertAndFocus(alertEl, "error", "لا توجد صلاحية لتعديل/حذف المناسبات لهذا المندوب."); return; } showAlertAndFocus(alertEl, "error", "تعذر الحذف حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } touchEventsRefresh(); if (options.kind === "death") { showAlertAndFocus(deathAlert, "success", "تم الحذف."); await loadDeathsForBranch(state.branch); return; } showAlertAndFocus(alertEl, "success", "تم الحذف."); await loadEventsForBranch(state.branch); } function isPermissionDeniedError(err) { const msgRaw = err && err.message != null ? String(err.message) : ""; const detailsRaw = err && err.details != null ? String(err.details) : ""; const codeRaw = err && err.code != null ? String(err.code) : ""; const low = (codeRaw + " " + msgRaw + " " + detailsRaw).toLowerCase(); return ( low.includes("row-level security") || low.includes("violates row-level security") || low.includes("permission denied") || low.includes("not authorized") || low.includes("not allowed") || low.includes("42501") ); } if (cancelHappyBtn) cancelHappyBtn.addEventListener("click", () =>resetHappyForm()); if (cancelSickBtn) cancelSickBtn.addEventListener("click", () =>resetSickForm()); if (cancelDeathBtn) cancelDeathBtn.addEventListener("click", () =>resetDeathForm()); if (sickPlace) sickPlace.addEventListener("change", updateSickPlaceVisibility); if (sickContactMethod) sickContactMethod.addEventListener("change", updateSickContactModeVisibility); if (sickVisitTimeFromHour) sickVisitTimeFromHour.addEventListener("change", () =>syncVisitTimeHidden(sickVisitTimeFromHour, sickVisitTimeFromMinute, sickVisitTimeFromPeriod, sickVisitTimeFrom)); if (sickVisitTimeFromMinute) sickVisitTimeFromMinute.addEventListener("change", () =>syncVisitTimeHidden(sickVisitTimeFromHour, sickVisitTimeFromMinute, sickVisitTimeFromPeriod, sickVisitTimeFrom)); if (sickVisitTimeFromPeriod) sickVisitTimeFromPeriod.addEventListener("change", () =>syncVisitTimeHidden(sickVisitTimeFromHour, sickVisitTimeFromMinute, sickVisitTimeFromPeriod, sickVisitTimeFrom)); if (sickVisitTimeToHour) sickVisitTimeToHour.addEventListener("change", () =>syncVisitTimeHidden(sickVisitTimeToHour, sickVisitTimeToMinute, sickVisitTimeToPeriod, sickVisitTimeTo)); if (sickVisitTimeToMinute) sickVisitTimeToMinute.addEventListener("change", () =>syncVisitTimeHidden(sickVisitTimeToHour, sickVisitTimeToMinute, sickVisitTimeToPeriod, sickVisitTimeTo)); if (sickVisitTimeToPeriod) sickVisitTimeToPeriod.addEventListener("change", () =>syncVisitTimeHidden(sickVisitTimeToHour, sickVisitTimeToMinute, sickVisitTimeToPeriod, sickVisitTimeTo)); if (saveHappyBtn) { saveHappyBtn.addEventListener("click", async () =>{ if (!state.branch) { showAlert(happyAlert, "error", "يلزم تسجيل دخول المندوب أولاً."); return; } const type = String(happyType && happyType.value ? happyType.value : "").trim(); const person = normalizePersonName(happyPerson && happyPerson.value ? happyPerson.value : ""); let dateValue = String(happyDate && happyDate.value ? happyDate.value : "").trim(); const detailsText = String(happyDetails && happyDetails.value ? happyDetails.value : "").trim(); if (!type || !person) { showAlert(happyAlert, "error", "يرجى اختيار نوع المناسبة وكتابة اسم الشخص."); return; } if (!dateValue) { dateValue = todayGregorianISO(); if (happyDate) happyDate.value = dateValue; } const dateLabel = dateValue ? formatDateISO(dateValue) : ""; const sb = getالخدمةClient(); if (!sb) { showAlert(happyAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } const looksLikeMessage = detailsText && (detailsText.split(/\s+/).filter(Boolean).length >= 4 || detailsText.includes("مبروك") || detailsText.includes("نبارك") || detailsText.includes("تهانينا")); const baseHappyText = formatEventText({ type, person }); const storedText = looksLikeMessage ? detailsText : baseHappyText; const showDays = clampVisibilityDays(happyVisibilityDays && happyVisibilityDays.value ? happyVisibilityDays.value : 7); let mediaValues = { imageUrl: "", videoUrl: "" }; try { mediaValues = await getHappyMediaValues(sb, editingHappy); } catch (e) { showAlert(happyAlert, "error", e && e.message ? e.message : "تعذر رفع المرفقات."); return; } const happyPayload = { v: 1, kind: "happy_notice", text: storedText || "", extra: looksLikeMessage ? "" : detailsText, imageUrl: mediaValues.imageUrl || "", videoUrl: mediaValues.videoUrl || "", showDays }; const patch = { branch_key: state.branch, type, person, date_label: dateLabel || null, event_date: dateValue || null, details: JSON.stringify(happyPayload), hospital_name: null, hospital_dept: null, contact_method: null, contact_phone: null, visit_date_from: null, visit_date_to: null, visit_time_from: null, visit_time_to: null }; if (editingHappy) { const pk = getEventPk(editingHappy); if (!pk) { showAlert(happyAlert, "error", "تعذر حفظ التعديل لأن معرف السجل غير متوفر."); return; } const res = await rpcUpdateFamilyEventRow(sb, pk, patch); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { showAlert( happyAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } if (isCaseTypesTextAndDateMismatchError(err)) { showAlert( happyAlert, "error", "تعذر حفظ التعديل حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } const msg = String(err && err.message ? err.message : ""); if (msg.toLowerCase().includes("not allowed")) { showAlert(happyAlert, "error", "لا توجد صلاحية لتعديل المناسبات لهذا المندوب."); return; } showAlert(happyAlert, "error", "تعذر حفظ التعديل حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } touchEventsRefresh(); resetHappyForm(); showAlert(happyAlert, "success", "تم حفظ التعديل."); await loadEventsForBranch(state.branch); return; } const row = { ...patch, created_at: new Date().toISOString() }; const res = await rpcInsertFamilyEventRow(sb, row); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { showAlert( happyAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } if (isCaseTypesTextAndDateMismatchError(err)) { showAlert( happyAlert, "error", "تعذر الإضافة حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } const msg = String(err && err.message ? err.message : ""); if (msg.toLowerCase().includes("not allowed")) { if (err && err.detail === "events_access") { const st = String(err.status || ""); const rid = String(err.requestId || ""); if (st === "pending") { showAlert(happyAlert, "error", rid ? `صلاحية المناسبات قيد المراجعة (رقم الطلب: ${rid}).` : "صلاحية المناسبات قيد المراجعة."); return; } if (st === "rejected") { showAlert(happyAlert, "error", rid ? `تم رفض صلاحية المناسبات (رقم الطلب: ${rid}).` : "تم رفض صلاحية المناسبات."); return; } if (st === "approved") { showAlert(happyAlert, "error", "الصلاحية معتمدة لكن بيانات الدخول لا تطابق بيانات الاعتماد. جرّب تسجيل خروج/دخول وتأكد من البريد والجوال."); return; } } showAlert(happyAlert, "error", "لا توجد صلاحية لإضافة المناسبات لهذا المندوب."); return; } showAlert(happyAlert, "error", "تعذر الإضافة حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } maybeOpenEmailDraft( "مناسبة جديدة (" + type + ")", ["مناسبة جديدة", "الفرع: " + state.branch, "النوع: " + type, "الاسم: " + person, dateLabel ? "التاريخ: " + dateLabel : "", storedText ? "النص: " + storedText : ""] .filter(Boolean) .join("\n") ); touchEventsRefresh(); resetHappyForm(); showAlert(happyAlert, "success", "تمت الإضافة."); await loadEventsForBranch(state.branch); }); } if (saveSickBtn) { saveSickBtn.addEventListener("click", async () =>{ if (!state.branch) { showAlert(sickAlert, "error", "يلزم تسجيل دخول المندوب أولاً."); return; } const type = String(sickType && sickType.value ? sickType.value : "").trim(); const person = normalizePersonName(sickPerson && sickPerson.value ? sickPerson.value : ""); let dateValue = String(sickDate && sickDate.value ? sickDate.value : "").trim(); if (!type || !person) { showAlert(sickAlert, "error", "يرجى اختيار نوع الحالة وكتابة اسم الشخص."); return; } if (!dateValue) { dateValue = todayGregorianISO(); if (sickDate) sickDate.value = dateValue; } const dateLabel = dateValue ? formatDateISO(dateValue) : ""; const place = sickPlace && sickPlace.value === "home" ? "home" : "hospital"; const hospitalNameValue = place === "hospital" ? String(sickHospitalName && sickHospitalName.value ? sickHospitalName.value : "").trim() : ""; const hospitalDeptValue = place === "hospital" ? String(sickHospitalDept && sickHospitalDept.value ? sickHospitalDept.value : "").trim() : ""; const homeCityValue = place === "home" ? normalizePersonName(sickHomeCity && sickHomeCity.value ? sickHomeCity.value : "") : ""; const homeAreaValue = place === "home" ? normalizePersonName(sickHomeArea && sickHomeArea.value ? sickHomeArea.value : "") : ""; const notesValue = String(sickNotes && sickNotes.value ? sickNotes.value : "").trim(); const contactMethodValue = String(sickContactMethod && sickContactMethod.value ? sickContactMethod.value : "visit").trim() || "visit"; const contactPhoneValue = String(sickContactPhone && sickContactPhone.value ? sickContactPhone.value : "").trim(); const visitDateFromLabel = String(sickVisitDateFrom && sickVisitDateFrom.value ? sickVisitDateFrom.value : "").trim(); const visitDateToLabel = String(sickVisitDateTo && sickVisitDateTo.value ? sickVisitDateTo.value : "").trim(); const visitTimeFromValue = String(sickVisitTimeFrom && sickVisitTimeFrom.value ? sickVisitTimeFrom.value : "").trim(); const visitTimeToValue = String(sickVisitTimeTo && sickVisitTimeTo.value ? sickVisitTimeTo.value : "").trim(); if (contactMethodValue === "visit") { if (!visitTimeFromValue || !visitTimeToValue) { showAlert(sickAlert, "error", "يرجى تحديد مواعيد الزيارة (من الساعة وإلى الساعة) أو اختر اتصال/واتساب."); return; } } else { if (!contactPhoneValue) { showAlert(sickAlert, "error", "يرجى إدخال رقم الجوال للتواصل (اتصال/واتساب)."); return; } } const showDays = clampVisibilityDays(sickVisibilityDays && sickVisibilityDays.value ? sickVisibilityDays.value : 7); const payload = { v: 1, kind: "health_notice", place, homeCity: homeCityValue, homeArea: homeAreaValue, notes: notesValue, hospitalName: hospitalNameValue, hospitalDept: hospitalDeptValue, showDays }; const details = JSON.stringify(payload); const sb = getالخدمةClient(); if (!sb) { showAlert(sickAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } const patch = { branch_key: state.branch, type, person, date_label: dateLabel || null, event_date: dateValue || null, details, hospital_name: hospitalNameValue || null, hospital_dept: hospitalDeptValue || null, contact_method: contactMethodValue || null, contact_phone: contactPhoneValue || null, visit_date_from: visitDateFromLabel || null, visit_date_to: visitDateToLabel || null, visit_time_from: visitTimeFromValue || null, visit_time_to: visitTimeToValue || null }; if (editingSick) { const pk = getEventPk(editingSick); if (!pk) { showAlert(sickAlert, "error", "تعذر حفظ التعديل لأن معرف السجل غير متوفر."); return; } const res = await rpcUpdateFamilyEventRow(sb, pk, patch); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { showAlert( sickAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } if (isCaseTypesTextAndDateMismatchError(err)) { showAlert( sickAlert, "error", "تعذر حفظ التعديل حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } const msg = String(err && err.message ? err.message : ""); if (msg.toLowerCase().includes("not allowed")) { showAlert(sickAlert, "error", "لا توجد صلاحية لتعديل المناسبات لهذا المندوب."); return; } showAlert(sickAlert, "error", "تعذر حفظ التعديل حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } touchEventsRefresh(); resetSickForm(); showAlert(sickAlert, "success", "تم حفظ التعديل."); await loadEventsForBranch(state.branch); return; } const row = { ...patch, created_at: new Date().toISOString() }; const res = await rpcInsertFamilyEventRow(sb, row); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { showAlert( sickAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } if (isCaseTypesTextAndDateMismatchError(err)) { showAlert( sickAlert, "error", "تعذر الإضافة حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } const msg = String(err && err.message ? err.message : ""); if (msg.toLowerCase().includes("not allowed")) { if (err && err.detail === "events_access") { const st = String(err.status || ""); const rid = String(err.requestId || ""); if (st === "pending") { showAlert(sickAlert, "error", rid ? `صلاحية المناسبات قيد المراجعة (رقم الطلب: ${rid}).` : "صلاحية المناسبات قيد المراجعة."); return; } if (st === "rejected") { showAlert(sickAlert, "error", rid ? `تم رفض صلاحية المناسبات (رقم الطلب: ${rid}).` : "تم رفض صلاحية المناسبات."); return; } if (st === "approved") { showAlert(sickAlert, "error", "الصلاحية معتمدة لكن بيانات الدخول لا تطابق بيانات الاعتماد. جرّب تسجيل خروج/دخول وتأكد من البريد والجوال."); return; } } showAlert(sickAlert, "error", "لا توجد صلاحية لإضافة المناسبات لهذا المندوب."); return; } showAlert(sickAlert, "error", "تعذر الإضافة حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } maybeOpenEmailDraft( "مناسبة جديدة (" + type + ")", [ "مناسبة جديدة", "الفرع: " + state.branch, "النوع: " + type, "الاسم: " + person, dateLabel ? "التاريخ: " + dateLabel : "", notesValue ? "ملاحظات: " + notesValue : "" ] .filter(Boolean) .join("\n") ); touchEventsRefresh(); resetSickForm(); showAlert(sickAlert, "success", "تمت الإضافة."); await loadEventsForBranch(state.branch); }); } if (saveDeathBtn) { saveDeathBtn.addEventListener("click", async () =>{ if (!state.branch) { showAlert(deathAlert, "error", "يلزم تسجيل دخول المندوب أولاً."); return; } const person = normalizePersonName(deathPerson && deathPerson.value ? deathPerson.value : ""); let dateValue = String(deathDate && deathDate.value ? deathDate.value : "").trim(); if (!person) { showAlert(deathAlert, "error", "يرجى كتابة اسم المتوفى."); return; } if (!dateValue) { dateValue = todayGregorianISO(); if (deathDate) deathDate.value = dateValue; } const dateLabel = dateValue ? formatDateISO(dateValue) : ""; const showDays = clampVisibilityDays(deathVisibilityDays && deathVisibilityDays.value ? deathVisibilityDays.value : 7); const payload = { v: 1, kind: "death_notice", prayerPlace: normalizePersonName(prayerPlace && prayerPlace.value ? prayerPlace.value : ""), prayerTime: normalizePersonName(prayerTime && prayerTime.value ? prayerTime.value : ""), burialPlace: normalizePersonName(burialPlace && burialPlace.value ? burialPlace.value : ""), burialTime: normalizePersonName(burialTime && burialTime.value ? burialTime.value : ""), condolencePlace: normalizePersonName(condolencePlace && condolencePlace.value ? condolencePlace.value : ""), condolenceTime: normalizePersonName(condolenceTime && condolenceTime.value ? condolenceTime.value : ""), phones: normalizePhonesForDisplay([ condolencePhone1 && condolencePhone1.value ? condolencePhone1.value : "", condolencePhone2 && condolencePhone2.value ? condolencePhone2.value : "" ]), notes: String(deathNotes && deathNotes.value ? deathNotes.value : "").trim(), showDays }; const details = JSON.stringify(payload); const sb = getالخدمةClient(); if (!sb) { showAlert(deathAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } const patch = { branch_key: state.branch, type: "death", person, date_label: dateLabel || null, event_date: dateValue || null, details, hospital_name: null, hospital_dept: null, contact_method: null, contact_phone: null, visit_date_from: null, visit_date_to: null, visit_time_from: null, visit_time_to: null }; if (editingDeath) { const pk = getEventPk(editingDeath); if (!pk) { showAlert(deathAlert, "error", "تعذر حفظ التعديل لأن معرف السجل غير متوفر."); return; } const res = await rpcUpdateFamilyEventRow(sb, pk, patch); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { showAlert( deathAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } if (isCaseTypesTextAndDateMismatchError(err)) { showAlert( deathAlert, "error", "تعذر حفظ التعديل حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } const msg = String(err && err.message ? err.message : ""); if (msg.toLowerCase().includes("not allowed")) { showAlert(deathAlert, "error", "لا توجد صلاحية لتعديل المناسبات لهذا المندوب."); return; } showAlert(deathAlert, "error", "تعذر حفظ التعديل حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } touchEventsRefresh(); resetDeathForm(); showAlert(deathAlert, "success", "تم حفظ التعديل."); await loadDeathsForBranch(state.branch); return; } const row = { ...patch, created_at: new Date().toISOString() }; const res = await rpcInsertFamilyEventRow(sb, row); if (!res.ok) { const err = res.error || { message: "unknown error" }; if (isRpcMissingError(err)) { showAlert( deathAlert, "error", "تعذر الحفظ حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } if (isCaseTypesTextAndDateMismatchError(err)) { showAlert( deathAlert, "error", "تعذر النشر حالياً، حاول لاحقاً أو تواصل مع الإدارة." ); return; } const msg = String(err && err.message ? err.message : ""); if (msg.toLowerCase().includes("not allowed")) { if (err && err.detail === "events_access") { const st = String(err.status || ""); const rid = String(err.requestId || ""); if (st === "pending") { showAlert(deathAlert, "error", rid ? `صلاحية المناسبات قيد المراجعة (رقم الطلب: ${rid}).` : "صلاحية المناسبات قيد المراجعة."); return; } if (st === "rejected") { showAlert(deathAlert, "error", rid ? `تم رفض صلاحية المناسبات (رقم الطلب: ${rid}).` : "تم رفض صلاحية المناسبات."); return; } if (st === "approved") { showAlert(deathAlert, "error", "الصلاحية معتمدة لكن بيانات الدخول لا تطابق بيانات الاعتماد. جرّب تسجيل خروج/دخول وتأكد من البريد والجوال."); return; } } showAlert(deathAlert, "error", "لا توجد صلاحية لإضافة المناسبات لهذا المندوب."); return; } showAlert(deathAlert, "error", "تعذر النشر حالياً، حاول لاحقاً أو تواصل مع الإدارة."); return; } maybeOpenEmailDraft( "خبر وفاة جديد", [ "خبر وفاة جديد", "الفرع: " + state.branch, "الاسم: " + person, dateLabel ? "التاريخ: " + dateLabel : "", payload.prayerPlace ? "مكان الصلاة: " + payload.prayerPlace : "", payload.prayerTime ? "وقت الصلاة: " + payload.prayerTime : "", payload.burialPlace ? "مكان الدفن: " + payload.burialPlace : "", payload.burialTime ? "وقت الدفن: " + payload.burialTime : "", payload.condolencePlace ? "مكان العزاء: " + payload.condolencePlace : "", payload.condolenceTime ? "وقت العزاء: " + payload.condolenceTime : "", payload.phones ? "الجوال: " + payload.phones : "", payload.notes ? "ملاحظات: " + payload.notes : "" ] .filter(Boolean) .join("\n") ); touchEventsRefresh(); resetDeathForm(); showAlert(deathAlert, "success", "تم نشر خبر الوفاة."); await loadDeathsForBranch(state.branch); }); } updateSickPlaceVisibility(); updateSickContactModeVisibility(); renderHappyEvents(); renderSickEvents(); renderDeaths();
