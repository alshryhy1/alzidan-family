...
`.trim();
  ...
  function buildRequestDetailsText(row) {
    const lines = [
      ...
      row.created_at
        ...
        : "",
      "",
      window.AlzidanRequestActions.requestMessageWithoutMediaLinks(row.message || ""),
    ].filter(
  ...
  function renderRequestRow(row) {
    ...
    window.AlzidanRequestActions.appendRequestMediaPreview(body, row.message || "");
    ...
    editBranchBtn.addEventListener("click", async () => {
      ...
      if (row.kind === "tree_card") {
        window.AlzidanRequestActions.openTreeCardEditor(row);
        return;
      }
      ...
      const message = window.AlzidanRequestActions.updateBranchInRequestMessage(
        row.message,
        branchKey,
        row.kind,
      );
    ...
    approveBtn.addEventListener("click", async () => {
      ...
      if (row.kind === "tree_card") {
        const imported = await window.AlzidanRequestActions.importTreeCardToTree(sb, token, row);
        if (!imported.ok) {
          showAlert("error", imported.message || "تعذر إضافة البطاقة للشجرة.");
          return;
        }
      } else if (row.kind === "event_card") {
        const published = await window.AlzidanRequestActions.publishEventCardRequest(sb, token, row);
        if (!published.ok) {
  ...
  if (window.AlzidanRequestActions && typeof window.AlzidanRequestActions.setReloadRequests === "function") {
    window.AlzidanRequestActions.setReloadRequests(loadRequests);
  }
