...
`
  ...
  function buildRequestDetailsText(row) {
    const lines = [
      ...
      row.created_at
        ...
        : "",
      "",
      requestMessageWithoutMediaLinks(row.message || ""),
    ].filter(
  ...
  function renderRequestRow(row) {
    ...
    appendRequestMediaPreview(body, row.message || "");
    ...
    editBranchBtn.addEventListener("click", async () => {
      ...
      const message = updateBranchInRequestMessage(
        row.message,
        branchKey,
        row.kind,
      );
    ...
    approveBtn.addEventListener("click", async () => {
      ...
      if (row.kind === "tree_card") {
        const imported = await importTreeCardToTree(sb, token, row);
        if (!imported.ok) {
          showAlert("error", imported.message || "تعذر إضافة البطاقة للشجرة.");
          return;
        }
      } else if (row.kind === "event_card") {
        const published = await publishEventCardRequest(sb, token, row);
        if (!published.ok) {
  ...
  (async function init() {
    ...
    if (adminToken) {
      ...
      if (
        ...
        window.AlzidanAdminViews.loadViewsStats().catch(() => {});
      loadSourceTreeRows().catch(() => {});
      if (delegateAuditDetails && delegateAuditDetails.open) {
