...
| 🟠 مرتفع | عدم تطابق المسار / علاقة مكسورة / **أسماء متشابهة** |
| 🟡 متوسط | ربط UUID / TREE-003 |
| 🟢 سليم | علاقات صحيحة |

**بطاقة الملخص:** حرج · يحتاج مراجعة · UUID · سليم.

### الأسماء المتشابهة (Possible Duplicates) — 2026-08-09و

بطاقة 🟠 تحت سلامة البيانات: أبناء **نفس الأب** تطابقت أسماؤهم بعد تطبيع العربية (همزة أ/إ/آ، ى↔ي، ة↔ه، إزالة التشكيل).

| عمود | معنى |
|------|------|
| الأب | الأب المشترك (`parent_person_id` إن وُجد، وإلا مسار الأب) |
| الاسم الأول / الثاني | ورقتا الاسم المشتبه |
| نسبة التشابه | 100% بعد التطبيع |
| الحالة | **يحتاج مراجعة** فقط |

**لا دمج تلقائي** — Analyze يعرض ملاحظات؛ ممنوع توليد SQL دمج من Health Center (Truth Before Speed).

التحقق: `npm run verify:spelling-dupes`
...
**حقن SQL:** `UPDATE … WHERE id = N;` فقط (تعليقات `--` · بلا `/* */` · بلا SELECT مسبق) — تجنّب timeout التصنيف القديم على صفوف مثل id=1500.
...
| أسماء متشابهة | مراجعة فقط — لا دمج تلقائي |
