...
function normalizeArabicDigitsToLatin(value) {
  return String(value == null ? "" : value).replace(/[٠-٩۰-۹]/g, function (ch) {
    ...
    if (code >= 0x06F0 && code <= 0x06F9) return String(code - 0x06F0); // Eastern Arabic-Indic
    return ch;
  });
}

function normalizePhone(v) {
  var digits = normalizeArabicDigitsToLatin(String(v || "")).replace(/[^0-9]/g, "").trim();
  if (!digits) return "";
...
function normalizeDelegateSecret(value) {
  return normalizeArabicDigitsToLatin(String(value || "")).trim();
}
...
function bindSilentDigitNormalize(el, mode) {
  ...
  const applyDigitsOnly = function () {
    ...
    const next = normalizeArabicDigitsToLatin(cur);
    if (next !== cur) el.value = next;
  };
  const applyPhone = function () {
    const next = normalizePhone(el.value || "");
    if (next && next !== String(el.value || "")) el.value = next;
    else applyDigitsOnly();
  };
  const onBlur = mode === "phone" ? applyPhone : applyDigitsOnly;
  el.addEventListener("input", applyDigitsOnly);
