...
(function (global) {
  ...
  function auditRequestJourney(children, pendingRequests) {
    ...
    var routingWeak = pending
      ...
      .map(function (r) {
        return {
          ...
          child_path: r.kind || "",
          parent: "",
          impact: "يؤثر على Workflow / قد يصل للطرف الخطأ",
        };
      });
    ...
    var checks = [
      {
        ...
        label: "هل كل أبناء الأب يظهرون في مصدر بيانات RX؟",
        mode: "auto",
        ...
        detail_ar:
          pathQueryGaps.length === 0
            ? "لا فجوة بين parent_person_id وعمود parent في العيّنة المفهرسة."
            : "أبناء لهم parent_person_id لكن عمود parent فارغ/منحرف — قد يختفون من قائمة الأبناء في RX.",
        impact: "لا يظهر ضمن أبناء الأب",
      ...
      },
      {
        ...
        detail_ar:
          searchHidden.length === 0
            ? "لا صفوف غير-جذر بـ parent=NULL في المسح."
            : "صفوف parent=NULL قد تُستبعد من بحث/تصفية بالمسار.",
      ...
      },
      {
        ...
        mode: "manual_smoke",
        mode_ar: "يدوي (دخان RX)",
        ok: null,
        count: null,
        detail_ar:
          "تحقق يدوي: اختر شخصًا موجودًا في RX → بوابة الهوية يجب أن تمنع «أضف فردًا». الآلي: عناقيد ورقة مكررة = " +
      ...
      },
      {
        ...
        mode: "auto",
        mode_ar: "آلي جزئي + دخان يدوي",
        ok: routingWeak.length === 0,
        count: routingWeak.length,
        detail_ar:
          routingWeak.length === 0
            ? pending.length
              ? "لا طلبات شجرة معلّقة بلا parent_person_id في العيّنة."
              : "لم تُحمَّل طلبات معلّقة — أعد التحديث بعد الدخول. الدخان اليدوي: اعتمد طلب إضافة وتحقق من المندوب/الفرع."
            : "طلبات شجرة معلّقة بلا parent_person_id — قد تتعطّل التوجيه أو الاعتماد (TREE-003).",
        impact: "يؤثر على Workflow",
        samples: routingWeak,
      },
      {
        id: "rx_identity_coverage",
        label: "تغطية person_id (وكيل اكتمال الهوية)",
        mode: "auto",
        mode_ar: "آلي (وكيل بيانات)",
        ok: withoutPid === 0,
        count: withoutPid,
    ...
    return {
      ...
      checks: checks,
      legend: {
        auto: "فحص آلي على بيانات الشجرة/الطلبات — ليس بديلاً عن دخان الواجهة.",
        manual_smoke:
          "خطوات يدوية قصيرة على الرئيسية (RX) بعد تحديث التقرير.",
      },
      manual_smoke_steps: [
        "افتح الرئيسية → أضف فردًا → أكّد شخصًا موجودًا: يجب ألا يُسمح بالإنشاء.",
        "اختر أبًا معروفًا (مثال: خميس) وتأكد أن كل الأبناء الظاهرين في مركز الصحة تحت parent_person_id يظهرون في القائمة.",
        "ابحث عن اسم معروف partial وfull — قارن العدد مع صفوف الشجرة لنفس الورقة.",
