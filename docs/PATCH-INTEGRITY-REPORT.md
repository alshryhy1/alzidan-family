...
امتداد دائم في **مركز صحة البيانات** (قراءة فقط):

| بطاقة | المعنى |
|-------|--------|
| parent = NULL | عمود `parent` فارغ (قد يكون `parent_name` ممتلئًا — انجراف عمودين) |
| الأب غير موجود | قيمة الأب لا تطابق أي `tree_children.name` / `child_name` في الفرع |
| عدم تطابق المسار | الأب المستخرج من `name` (إزالة آخر مقطع) ≠ عمود `parent` |
| person_id مكرر | نفس المعرّف على أكثر من صف |
| أبناء بدون أب صالح | اتحاد الأعطال الهيكلية |

**كيف تفتح التقرير:** لوحة الإدارة → مركز صحة البيانات → «تحديث التقرير» → اضغط بطاقة الفئة لعرض الصفوف.
