...
const eventRow = typeof Events.buildFamilyEventRow === "function" ? Events.buildFamilyEventRow({ source: "public_form", requestId: payload.requestId, branch: payload.branch, type: payload.type, person: payload.person, dateLabel: payload.dateLabel, text: payload.text, place: payload.place, imageUrl: payload.imageUrl, videoUrl: payload.videoUrl, createdAt: payload.createdAt }) : null;
lines.push(JSON.stringify({ v: 1, kind: "event_card", event: eventRow, submitter: payload }, null, 2));
return lines.join("\n");
} updateSubmitLinks();
openPanelFromHash();
