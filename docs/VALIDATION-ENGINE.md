...
| سلامة الهيكل | حماية بنية الشجرة — **رفض `parent=NULL` / أب ناقص قبل أي كتابة** |
...
الكتابة على `tree_children` حصرًا عبر **Tree Engine** بعد نتيجة تحقق مقبولة (إضافة Single Write — [`PLATFORM-PRINCIPLES.md`](./PLATFORM-PRINCIPLES.md)).
...
النظام يتحقق (هذه الطبقة) ويربط ويقرّر قابلية التطبيق؛ الكتابة عبر Workflow + Validation ثم **Tree Engine فقط** (**Single Write Rule** + كاتب وحيد).

**ملاحظة مواءمة Family Engine:** اليوم بعض مسارات الاعتماد/المندوب ما زالت تستدعي RPC مباشرة (دين موثّق في [`PATCH-1-WRITE-PATHS.md`](./PATCH-1-WRITE-PATHS.md)). Validation يجب أن ترفض أي حمولة تُسقط `parent` قبل أن تصل لتلك المسارات — والحارس في `AlzidanTreeEngine.prepareChildWriteRow` طبقة دفاع أخيرة حتى اكتمال المواءمة.
---
