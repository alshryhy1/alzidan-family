...
drop policy if exists "tree_children_select_public" on public.tree_children; create policy "tree_children_select_public" on public.tree_children for select to anon, authenticated using ( lower(btrim(coalesce(gender, ''))) not in ('daughter', 'female', 'f', 'أنثى', 'انثى', 'ابنة', 'بنت') ); revoke insert, update, delete on table public.tree_children from anon, authenticated;
grant select on table public.tree_children to anon, authenticated; create or replace function public.admin_tree_children_list_v1(p_token text, p_branch_key text) returns setof public.tree_children language plpgsql security definer set search_path = public as $list$ declare v_branch text; begin if not public.admin_token_ok_v1(p_token) then raise exception 'not allowed'; end if; v_branch := nullif(btrim(coalesce(p_branch_key, '')), ''); if v_branch is null then return; end if; return query select c.* from public.tree_children c where c.branch_key = v_branch order by c.id limit 5000; end; $list$; grant execute on function public.admin_tree_children_list_v1(text, text) to anon, authenticated; create or replace function public.tree_children_list_v1(p_branch_key text, p_phone text, p_email text, p_secret_hash text) returns setof public.tree_children language plpgsql security definer set search_path = public as $list$ declare v_branch text; begin v_branch := nullif(btrim(coalesce(p_branch_key, '')), ''); if v_branch is null then return; end if; if not public.tree_delegate_allowed_v1(v_branch, p_phone, p_email, p_secret_hash) then raise exception 'not allowed'; end if; return query select c.* from public.tree_children c where c.branch_key = v_branch order by c.id limit 5000; end; $list$; grant execute on function public.tree_children_list_v1(text, text, text, text) to anon, authenticated; create or replace function public.tree_delegate_allowed_v1( p_branch_key text, p_phone text, p_email text, p_secret_hash text
) returns boolean
language sql
stable
security definer
