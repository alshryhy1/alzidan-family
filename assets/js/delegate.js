...
function readDelegateSessionFromStorage() {
  try {
    ...
    return {
      ...
      phone: String(parsed.phone || "").trim() || normalizePhone(parsed.phone || ""),
      rpcPhone: String(parsed.rpcPhone || "").trim(),
...
function saveDelegateSession(branchKey, phone, email, secretHash, name, rpcPhone) {
  ...
  delegateRuntimeAuth = {
    ...
    rpcPhone: String(rpcPhone || (prev && prev.rpcPhone) || rawPhone || "").trim(),
...
function delegatePhoneAttempts(phone) {
  ...
  push(raw);
  const canon = typeof normalizePhone === "function" ? normalizePhone(raw) : raw;
  push(canon);
  const extra =
    typeof phoneCandidates === "function" ? phoneCandidates(raw) : [];
  (Array.isArray(extra) ? extra : []).forEach(push);
  if (canon && canon !== raw && typeof phoneCandidates === "function") {
    (phoneCandidates(canon) || []).forEach(push);
  }
  return seen;
}

let delegatePhoneResolveInflight = null;
async function resolveDelegateRpcPhone(session) {
  if (!session) return "";
  const cached = String(session.rpcPhone || "").trim();
  if (cached) return cached;
  if (delegatePhoneResolveInflight) return delegatePhoneResolveInflight;
  delegatePhoneResolveInflight = (async () => {
    const phones = delegatePhoneAttempts(session.phone);
    const sb = typeof getالخدمةClient === "function" ? getالخدمةClient() : null;
    if (!sb || !phones.length) return phones[0] || String(session.phone || "").trim();
    const tryAllowed = async (p) => {
      const write = await sb.rpc("tree_delegate_allowed_v1", {
        p_branch_key: session.branch,
        p_phone: p,
        p_email: session.email,
        p_secret_hash: session.secretHash,
      });
      return !write.error && write.data === true;
    };
    const tryAccess = async (p) => {
      const info = await sb.rpc("check_tree_delegate_access", {
        p_branch_key: session.branch,
        p_phone: p,
        p_email: session.email,
        p_secret_hash: session.secretHash,
      });
      return !info.error && info.data && info.data.allowed === true;
    };
    for (let i = 0; i < phones.length; i++) {
      try {
        if (await tryAllowed(phones[i])) return phones[i];
      } catch (_) {}
    }
    for (let j = 0; j < phones.length; j++) {
      try {
        if (await tryAccess(phones[j])) return phones[j];
      } catch (_) {}
    }
    return phones[0];
  })();
  try {
    const picked = await delegatePhoneResolveInflight;
    const p = String(picked || "").trim();
    if (p) {
      session.rpcPhone = p;
      delegateRuntimeAuth = session;
      writeDelegateSessionToStorage(session);
    }
    return p;
  } finally {
    delegatePhoneResolveInflight = null;
  }
}

function persistDelegateRpcPhone(phone) {
  const p = String(phone || "").trim();
  if (!p) return;
  const session = loadDelegateSession();
  if (!session) return;
  if (session.rpcPhone === p) return;
  session.rpcPhone = p;
  delegateRuntimeAuth = session;
  writeDelegateSessionToStorage(session);
}
...
async function getDelegateRpcAuth() {
  ...
  if (!session) {
    return {
      ok: false,
      reason: "no_session",
      error: { message: "سجّل دخول المندوب ثم أعد الحفظ.", reason: "no_session" },
    };
  }
  ...
  if (!secretHash) {
    return {
      ok: false,
      reason: "hash_failed",
      error: { message: "تعذر قراءة الرقم السري للجلسة. سجّل الخروج ثم الدخول." },
    };
  }
  const phones = delegatePhoneAttempts(
    session.rpcPhone || session.phone
  ).concat(delegatePhoneAttempts(session.phone));
  const uniquePhones = [];
  phones.forEach((p) => {
    if (p && uniquePhones.indexOf(p) < 0) uniquePhones.push(p);
  });
  const resolved = await resolveDelegateRpcPhone(session);
  const phone = resolved || uniquePhones[0] || session.phone;
  if (phone) {
    const idx = uniquePhones.indexOf(phone);
    if (idx >= 0) uniquePhones.splice(idx, 1);
    uniquePhones.unshift(phone);
  }
  return {
    ok: true,
    branch: session.branch,
    phone,
    phones: uniquePhones.length ? uniquePhones : [phone].filter(Boolean),
    email: session.email,
    secretHash,
  };
}
window.getDelegateRpcAuth = getDelegateRpcAuth;
window.AlzidanDelegateTreeWrite = {
  ...
  updateBirthOrder: async function (child, birthOrder) {
    ...
    return rpcUpdateTreeChildRow(
      sb,
      (session && session.branch) || (child && child.branch_key) || "",
...
function findStablePersonId(nodeId) { const wanted = normalizePersonName(nodeId || ""); if (!wanted) return ""; const meta = state.pathToRow && state.pathToRow[wanted] ? state.pathToRow[wanted] : null; if (meta && meta.person_id) return normalizePersonName(meta.person_id); if (meta && state.pathToRow["pid:" + normalizePersonName(meta.person_id)]) return normalizePersonName(meta.person_id); const lists = Object.values(state.children || {}); for (let i = 0; i< lists.length; i++) { const list = Array.isArray(lists[i]) ? lists[i] : []; for (let j = 0; j< list.length; j++) { const child = list[j] || {}; if (normalizePersonName(child.name || "") !== wanted) continue; return normalizePersonName(child.personId || ""); } } return ""; } async function rpcInsertTreeChildRow(sb, row) { const auth = await getDelegateRpcAuth(); if (!auth.ok) return { ok: false, error: { message: auth.reason } }; const r = row || {}; const key = normalizePersonName(r.branch_key || ""); const parentFull = normalizePersonName(r.parent_name || r.parent || ""); const childFull = normalizePersonName(r.child_name || r.name || ""); if (!key || !parentFull || !childFull) return { ok: false, error: { message: "missing key" } }; let parentPersonId = normalizePersonName(r.parent_person_id || "") || findStablePersonId(parentFull); if (!parentPersonId && window.AlzidanCanonicalPerson && typeof window.AlzidanCanonicalPerson.attachParentPersonId === "function") { const attached = window.AlzidanCanonicalPerson.attachParentPersonId(r, state.pathToRow, parentFull, canonicalHelpers()); parentPersonId = normalizePersonName(attached.parent_person_id || ""); } const rpcRow = Object.assign({}, r, { parent_name: parentFull, parent: parentFull, child_name: childFull, name: childFull }); if (parentPersonId) rpcRow.parent_person_id = parentPersonId; const gender = typeof normalizeTreeChildGender === "function" ? normalizeTreeChildGender(rpcRow.gender) : ""; if (gender) rpcRow.gender = gender; const { data, error } = await sb.rpc("tree_children_insert_v1", { p_branch_key: key, p_parent_name: parentFull, p_child_name: childFull, p_phone: auth.phone, p_email: auth.email, p_secret_hash: auth.secretHash, p_row: rpcRow }); if (error) { if (isOnConflictConstraintError(error)) { return { ok: false, error }; } return { ok: false, error }; } if (data === true) { if (gender) { const stamped = await sb.rpc("tree_children_set_gender_v1", { p_branch_key: key, p_child_name: childFull, p_gender: gender, p_phone: auth.phone, p_email: auth.email, p_secret_hash: auth.secretHash }); if (stamped.error && !isRpcMissingError(stamped.error)) return { ok: false, error: stamped.error }; } return { ok: true }; } try { const info = await sb.rpc("check_tree_delegate_access", { p_branch_key: key, p_phone: auth.phone, p_email: auth.email, p_secret_hash: auth.secretHash }); const denied = (!info.error && info.data && typeof info.data === "object") ? info.data : {}; return { ok: false, error: { message: "not allowed", reason: denied.reason || denied.status || "not allowed", role_key: denied.role_key || null, operation_key: denied.operation_key || "tree.write", detail: formatDelegateDeniedError(denied, "tree") } }; } catch (e) { return { ok: false, error: { message: "not allowed" } }; } } async function rpcUpdateTreeChildRow(sb, branchKey, parentId, childId, patch, personId) {
      ...
      error: {
        message:
          auth.reason === "no_session"
            ? "سجّل دخول المندوب ثم أعد الحفظ."
            : "صلاحية المندوب غير مكتملة. سجّل الخروج ثم الدخول بالجوال والرقم السري.",
        reason: auth.reason,
      },
    };
  }
  const parentFull = normalizePersonName(parentId || "");
  const childFull = normalizePersonName(childId || "");
  const parentStored = getLeafStoredNameFromNodeId(parentFull);
  const childStored = getLeafStoredNameFromNodeId(childFull);
  const parentBase = getLeafBaseNameFromNodeId(parentFull);
  const childBase = getLeafBaseNameFromNodeId(childFull);
  if (!(parentFull || parentStored) || !(childFull || childStored)) {
    return { ok: false, error: { message: "missing key" } };
  }
  const stablePersonId = normalizePersonName(personId || "") || findStablePersonId(childFull || childId);
  const rpcPatch = stablePersonId ? { ...(patch || {}), person_id: stablePersonId } : (patch || {});
  const pairs = [];
  const pushPair = (parentName, childName) => {
    const parent = normalizePersonName(parentName || "");
    const child = normalizePersonName(childName || "");
    if (!parent || !child) return;
    const sig = parent + "\0" + child;
    if (pairs.some((x) => x[0] === parent && x[1] === child)) return;
    pairs.push([parent, child]);
  };
  pushPair(parentFull, childFull);
  pushPair(parentStored, childStored);
  pushPair(parentBase, childBase);
  const branches = [];
  const pushBranch = (b) => {
  ...
  if (!branches.length) return { ok: false, error: { message: "missing key" } };
  const phones =
    auth.phones && auth.phones.length ? auth.phones : [auth.phone].filter(Boolean);
  const call = async (branch, parentName, childName, phone) =>
    sb.rpc("tree_children_update_v1", {
      p_branch_key: branch,
  ...
  for (let p = 0; p < phones.length; p++) {
    for (let b = 0; b < branches.length; b++) {
      for (let i = 0; i < pairs.length; i++) {
        last = await call(branches[b], pairs[i][0], pairs[i][1], phones[p]);
        if (last && last.error) return { ok: false, error: last.error };
        if (last && last.data === true) {
          persistDelegateRpcPhone(phones[p]);
          return { ok: true };
        }
      }
    }
...
loginBtn.addEventListener("click", async () =>{
  ...
  if (sbResult.ok && sbResult.status === "approved") {
    ...
    saveDelegateSession(
      ...
      sbResult.secretHash,
      loginName,
      sbResult.rpcPhone || sbResult.phone || phone
    );
...
async function familyApiUpdateChild(payload) {
  ...
  const deceased = !!payload.deceased;
  const hijriInput = normalizeArabicDigitsToLatin(String(payload.hijri || "").trim());
  const gregInput = String(payload.greg || "").trim();
  const yearOnly = /^(\d{4})$/.exec(hijriInput);
  let finalHijri = "";
  let finalGreg = "";
  let birthYear = null;
  if (yearOnly && !gregInput) {
    birthYear = normalizeBirthYear(yearOnly[1]);
    if (!birthYear) return { ok: false, message: "سنة الميلاد الهجرية غير صحيحة." };
    finalHijri = String(birthYear);
  } else {
    const hijriNorm = hijriInput ? normalizeHijriDateISO(hijriInput) : "";
    const gregNorm = gregInput ? normalizeGregorianDateISO(gregInput) : "";
    ...
    finalHijri = hijriNorm;
    finalGreg = gregNorm;
    if (finalHijri && !finalGreg) finalGreg = hijriToGregorianISO(finalHijri);
    if (finalGreg && !finalHijri) finalHijri = gregorianToHijriISO(finalGreg);
    if (finalHijri && !finalGreg) return { ok: false, message: "تعذر تحويل التاريخ الهجري إلى ميلادي." };
    if (finalGreg && !finalHijri) return { ok: false, message: "تعذر تحويل التاريخ الميلادي إلى هجري." };
    birthYear = finalHijri ? normalizeBirthYear(finalHijri.slice(0, 4)) : null;
  }
